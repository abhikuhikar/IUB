...
def emission(test, label):
    m = 0
    N = np.size(label)
    p = 0.35#error(test) + 0.001#m/N
    if np.count_nonzero(label) == 0:
        if (1-p)*np.count_nonzero(test) <= 5:
            return 1e-60
        else:
            return 1e-100
    else:
        m = np.count_nonzero(test == label)
    #p = 0.40#np.random.uniform(0.5,0.6,1)
...
transition = {}
transitionMatrix = np.zeros((72,72))
char_init = {}
char_init_probs = {}
...
train_letters_mat = {key : toMatrix(val) for key,val in train_letters.items()}
ground_labels = list(train_letters.keys())
test_letters = load_letters(test_img_fname)
test_letters_mat = [toMatrix(l) for l in test_letters]
letters = read_data(train_txt_fname)
train(letters)
...
def test():
    ...
    for i in range(19):
        ...
        test_letters_mat = [toMatrix(l) for l in test_letters]
        sequence2 = simple()
        print ("Simple: " + ''.join(sequence2))
        sequence3 = hmm_viterbi(test_letters_mat)
        print ("Viterbi: " + ''.join(sequence3))
...
sequence3 = hmm_viterbi(test_letters_mat)
print ("Viterbi: " + ''.join(sequence3))
print ("Final Answer: " + ''.join(sequence3))
